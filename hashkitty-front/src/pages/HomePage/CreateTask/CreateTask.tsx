...
> {
                     ...
                     >
                        <div className="grid2Fr">
                           <div>
                              ...
                              <InputHashlist
                                 {...inputOptions}
                                 buttonClick={this.toggleHashlistCreation}
                                 importMessage={
                                    this.state.importHashlistSuccessMessage
                                 }
                              />
                           </div>
                           <RenderTemplateRadio
                        ...
                        <Button type="submit" className="submitInputCreateTask">
                           Create task
                        </Button>
                     </form>
                  </div>
   ...
   private get form(): newTaskFormData {
      return {
         ...
         formBreakpointGPUTemperature: this.state.formBreakpointGPUTemperature,
         ...
         if (target.name === 'formAttackModeId') {
            if (this.state.templateCheckboxIsChecked) {
               const templateTask = this.state.templateTasks.find(
                  templateTask => {
                     return (
                        templateTask.id === this.state.templateTaskCheckBoxId
                     );
                  },
               );
               value =
   ...
   private handleSubmit = (event: FormEvent<HTMLFormElement>) => {
      ...
      if (!this.inputsError.hasErrors) {
         ...
         const options: ApiOptionsFormData = {
            ...
            breakpointGPUTemperature: this.state.formBreakpointGPUTemperature,
   ...
   private handleTemplateRadio = (
      ...
      if (!template || this.state.templateTaskCheckBoxId === template.id) {
         this.setState({
            templateTaskCheckBoxId: -1,
            templateCheckboxIsChecked: !this.state.templateCheckboxIsChecked,
            ...defaultFormData,
         });
      } else {
         this.setState({
            ...
            formWorkloadProfile: template.options.workloadProfileId.profileId,

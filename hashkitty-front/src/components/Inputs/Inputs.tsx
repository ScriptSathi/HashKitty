...
export function InputAttackModes({
   state,
   handleInputChange,
   biggerFonts,
}: {
   ...
   return state.attackModes.length > 0 ? (
      <div style={{ marginTop: 30 }}>
         ...
         <p
            className={
               state.formHasErrors ? 'isError noMargin' : 'hideBlock noMargin'
            }
         >
...
export function RenderTemplateRadio({
   list,
   handleOnClick,
   state,
}: {
                  ...
                  return (
                     ...
                     >
                        <input
                           className="inputRadio"
                           type="radio"
                           checked={state.templateTaskCheckBoxId === elem.id}
...
export function RenderAdvancedConfigButton({
...
export function InputRules({
   ...
   return (
      <label className="noMarginBottom labelsTitles">
         ...
         <p
            className={
               state.formHasErrors && state.inputsErrorCheck.formName.isError
                  ? 'isError noMargin'
                  : 'hideBlock noMargin'
            }
         >
...
export function InputWorkloadProfiles({
   state,
   handleInputChange,
}: {
   state: InputsState<
...
export function InputHashtypes({
   state,
   handleInputChange,
}: {
   state: InputsState<
      'formHashtypeName',
      Pick<newHashlistFormData, 'formHashtypeName'> & Pick<TDBData, 'hashtypes'>
   >;
   handleInputChange: (event: ChangeEvent<HTMLInputElement>) => void;
}) {
   return (
...
export function InputBreakpointTemp({
   state,
   handleInputChange,
}: {
   state: InputsState<
...
export function InputKernelOpti({
   state,
   handleInputChange,
}: {
   state: InputsState<

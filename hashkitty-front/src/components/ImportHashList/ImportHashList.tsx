...
type ImportHashListProps = {
   handleImportHasSucced: Dispatch<
      SetStateAction<{ message: string; color: string }>
   >;
};
...
> {
   ...
   public render() {
      return (
         // TODO A refaire
         <BackgroundBlur isToggled toggleFn={() => {}} centerContent>
                     ...
                     <div className="marginLeft30 marginMinus5Up">
                        <p
                           className={
                              ...
                              this.state.inputsErrorCheck.formHashlist.isError
                                 ? 'isError labelsTitles noMargin'
                                 : 'hideBlock noMargin'
                           }
                        >
                           {this.state.inputsErrorCheck.formHashlist.message}
                        </p>
                        <DragNDrop setFile={this.setFile} />
                     </div>
                  </div>
                  <div className="ImportHashList__button">
                     <Button type="submit" className="ImportHashList__submit">
                        Import Hashlist
                     </Button>
                  </div>
               </form>
   ...
   private submitForm(): void {
      ...
      fetch(Constants.apiPOSTAddList, requestOptions)
         ...
         .then(res => {
            ...
            this.props.handleImportHasSucced(status);
         });
   }

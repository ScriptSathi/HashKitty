...
export class Factory {
   ...
   public async fakeTasks(numberOfFakeTasks: number): Promise<void> {
      for (const i in [...Array(numberOfFakeTasks).keys()]) {
         ...
         if (Math.random() < 0) {
            ...
            const templateTasks = await this.createListIfNotExist<TemplateTask>(
               TemplateTask,
            ...
            const templateTask =
               templateTasks[this.getRandomValueInRange(templateTasks.length)];
         ...
         task.description = Math.random() < 0.5 ? this.makeFakeString(50) : '';
   ...
   private randomDate(year = 2021) {
      ...
      return new Date(
         start.getTime() +
            Math.floor(Math.random() * (new Date().getTime() - start.getTime()))
      );
   }
   ...
   private async createFakeOption(): Promise<Options> {
      ...
      const workloadProfile = await this.createListIfNotExist<WorkloadProfile>(
         WorkloadProfile,
      ...
      options.rules = this.makeFakeString(20);
      ...
      options.workloadProfileId =
         workloadProfile[this.getRandomValueInRange(workloadProfile.length)].id;
